...
class Secure extends Front_Controller {
	...
	function login($ajax = false)
	{
		...
		if ($submitted)
		{
			...
			$login		= $this->Jobs_model->member_login($email, $password, $remember);

			// read session for define the member is job seeker or employers
			$this->customer = $this->go_cart->customer();
			...
			if ($login)
			{
				if ($redirect == '')
				{
					...
					if($this->customer['type'] == 'E'){
						$redirect = 'secure/employer_account';
					}else if($this->customer['type'] == 'J'){
						$redirect = 'secure/jobseeker_account';
					}else{
						$redirect = 'secure/jobseeker_account';
					}										
				}
	...
	{	
		...
		*/
		...
		$data['page_title']	= lang('employers_registration');
		//default values are empty if the customer is new
		...
		/* $this->form_validation->set_rules('prefix', 'lang:prefix', 'trim');
		$this->form_validation->set_rules('suffix', 'lang:suffix', 'trim');
		$this->form_validation->set_rules('other_titles', 'lang:other_titles', 'trim'); */		
		$this->form_validation->set_rules('email', 'lang:address_email', 'trim|required|valid_email|max_length[128]|callback_check_email');				
		/* $this->form_validation->set_rules('username', 'lang:username', 'trim|required|max_length[128]'); */
		$this->form_validation->set_rules('password', 'lang:password', 'required|min_length[6]|sha1');
		$this->form_validation->set_rules('confirm', 'lang:confirm_password', 'required|matches[password]');		
		$this->form_validation->set_rules('company', 'lang:address_company', 'trim|max_length[128]');
		$this->form_validation->set_rules('phone', 'lang:address_phone', 'trim|required|max_length[32]');
		/* $this->form_validation->set_rules('fax', 'lang:address_fax', 'trim|required|max_length[32]'); */
		$this->form_validation->set_rules('address1', 'lang:address:address1', 'trim|required');
		$this->form_validation->set_rules('address2', 'lang:address:address2', 'trim');
		$this->form_validation->set_rules('city', 'lang:address:city', 'trim|required|max_length[32]');
		$this->form_validation->set_rules('state_id', 'lang:address:state', 'trim|required|numeric');
		...
		/* $this->form_validation->set_rules('company_type', 'lang:address:company_type', 'required'); */
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$this->view('employer_register', $data);
		}
		else
		{
			$save['id']		= false;
			...
			/* $save['prefix']				= $this->input->post('prefix');
			 $save['suffix']				= $this->input->post('suffix');
			$save['other_titles']		= $this->input->post('other_titles'); */
			$save['company']			= $this->input->post('company');
			...
			$save['id_state']			= $this->input->post('state_id');
			// Hardcode first since market is in Malaysia and data 129 is came from countries table
			$save['id_country']			= $this->input->post('country_id');
			$save['postcode']			= $this->input->post('zip');
			...
			// save the employer info and get their new id
			$id = $this->Jobs_model->save_members($save);
	...
	function register_jobseeker()
	{
		...
		*/
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$this->view('jobseeker_register', $data);
		}
		else
		{
			$save['id']		= false;
			...
			// save the customer info and get their new id
			$id = $this->Customer_model->save_members($save);
	...
	function employer_account($offset=0)
	{
		//make sure they're logged in
		$this->Customer_model->is_logged_in('secure/employer_account/');
			
		//get username and password 
		$data['customer']			= (array)$this->Jobs_model->get_member($this->customer['id']);		
		//get pernal details
		$data['employer'] 			= (array)$this->Jobs_model->get_member_detail($this->customer['id']);
		
		$data['country_id']		= $data['employer']['id_country'];
		$data['state_id']		= $data['employer']['id_state'];
		...
		$config['base_url'] = site_url('secure/employer_account');			
		//if they're logged in, then we have all their acct. info in the cookie.
		...
		*/	
		...
		$this->form_validation->set_rules('phone', 'lang:address_phone', 'trim|required|max_length[32]');
		/* $this->form_validation->set_rules('fax', 'lang:address_fax', 'trim|required|max_length[32]'); */
		$this->form_validation->set_rules('address1', 'lang:address:address1', 'trim|required|max_length[128]');
		$this->form_validation->set_rules('address2', 'lang:address:address2', 'trim|max_length[128]');
		$this->form_validation->set_rules('city', 'lang:address:city', 'trim|required|max_length[32]');
		$this->form_validation->set_rules('state_id', 'lang:address:state', 'trim|required|numeric');
		$this->form_validation->set_rules('country_id', 'lang:address:country', 'trim|required|numeric');
		$this->form_validation->set_rules('zip', 'lang:address:zip', 'trim|required|max_length[32]');
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$this->view('employer_account', $data);
		}
		else
		{
			$customer = array();
			...
			/* $customer['prefix']				= $this->input->post('prefix');
			$customer['suffix']				= $this->input->post('suffix');
			$customer['other_titles']		= $this->input->post('other_titles'); */
			$customer['company']			= $this->input->post('company');			
			...
			$customer['id_state']			= $this->input->post('state_id');
			$customer['id_country']			= $this->input->post('country_id');
			$customer['postcode']			= $this->input->post('zip');
			/* $customer['company_type']		= $this->input->post('company_type'); */			
			$customer['email_subscribe']		= intval((bool)$this->input->post('email_subscribe'));						
			...
			$this->go_cart->save_customer($this->customer);			
			$this->Jobs_model->save_members($customer);			
			$this->session->set_flashdata('message', lang('message_account_updated'));
	...
	function update_employer()
	{
		...
		*/
		...
		$this->form_validation->set_rules('facebook_account', 'Facebook Account', 'trim');
		$this->form_validation->set_rules('twitter_account', 'Twitter Account', 'trim');
		$this->form_validation->set_rules('google_account', 'Google Account', 'trim');
		...
		else
		{
			...
			$save['facebook_account']		= $this->input->post('facebook_account');
			$save['twitter_account']		= $this->input->post('twitter_account');
			$save['google_account']			= $this->input->post('google_account');
	...
	{
		//make sure they're logged in
		$this->Customer_model->is_logged_in('secure/change_password/');
		...
		*/
		...
		else
		{
			...
			redirect('secure/change_password');
		}
	}
	
	
	
	function jobseeker_account($offset=0)
	{
		//make sure they're logged in
		$this->Customer_model->is_logged_in('secure/jobseeker_account/');		
		...
		*/
		...
		if ($this->form_validation->run() == FALSE)
		{
			$this->view('jobseeker_account', $data);
		}
		else
		{
			$customer = array();
